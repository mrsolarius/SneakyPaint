...
Afin de respected au mieux les principes de KIS et de maintenabilité, nous avons choisi de suivre les patrons de conception suivants :
...
### Concepts généraux
L'application est principalement baser sur la classe whiteboard qui hérite de la classe JPanel.
C'est elle qui stocks les formes et qui les affiche.
Pour supporter ces fonctionnalités, elle est au centre de deux patron :
- Le patron command pour l'historisation des actions
- Le patron state pour gérer les comportements de l'application

Le whiteboard dispose d'un tableau de formes, c'est formes sont le deuxième centre de l'application.
En effet, ce sont elles qui sont affichées et qui sont sauvegardées.
Elles sont, elle aussi, au centre de plusieurs patrons :
...
L'export en XML ou JSON sont gérées par des classes utilitaires.
...
Elles sont organisées autours d'héritage multiple de classe abstraite afin de factoriser les eventuel code dupliqué.
Par exemple, les commande `AddSquareCommand`, `AddCircleCommand` et `AddRectangleCommand` hérite de la classe `AbstractAddShapeCommand` qui implémente les méthodes `execute`, `undo` et `redo`.
Elle leur demande juste d'implementer une méthode `createShape` qui permet de créer la forme à ajouter.


L'historisation des actions est gérée par la classe `History` qui stocke les commandes dans une liste et sauvgarde l'index de la dernière commande exécutée.
...
Et évidament, l'execution des methodes `undo` et `redo` de l'historique sont gérée directement par des commandes.
...
Chaque état ne dispose pas des même autorisations.
...
Ici les états SelectMode, AddCircle, AddTriangle et AddSquare non besoin de gérer que le callback du clique de la souris.
...
Dans l'état `AddSquare`, lorsque l'on clique sur la souris, la commande `AddSquareCommand` est exécutée et ajouter à l'historique.

Lorsque l'on clique sur le bouton `Select` par exemple, le whiteboard passe dans l'état `SelectMode`.

Dans l'état `SelectMode`, lorsque l'on clique sur la souris, la commande `SelectCommand` est exécutée et ajouter à l'historique si on à cliquer sur une forme.

La commande `SelectCommand` va passer la forme sélectionnée dans l'état `Selected`.

Lorsque l'event `MouseDragged` est déclenché, le whiteboard passe dans l'état `MoveMode` et la commande `MoveCommand` est exécutée puis ajouté à l'historique lorsque l'on relache le clique de la souris.

La commande `MoveCommand` va editer la position des formes sélectionnées.


## Conclusion
Ce projet nous a permis d'experimenter avec les différents patrons de conception.
Il nous à aussi apris à faire du clean code et à utiliser les tests unitaires pour vérifier le bon fonctionnement de notre code.
Au final ce projet et un bon exemple de ce que l'on peut faire avec les patrons de conception.
